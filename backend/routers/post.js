...
Router.post("/api/createPosts", async (req, res) => {
    ...
    try {
        ...
        const result = await postFunction.createPost({userId, createTimestamp, lastUpdateTimestamp, interestid, header, description, visibilityBoolean, active});
       
        res.status(201).json({ message: "Post added successfully", commentId: result.insertId });
    } catch (error) {
