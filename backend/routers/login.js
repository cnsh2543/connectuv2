...
Router.post('/', async (request, response, next) => {
  try {
    ...
    const [rows] = await pool.query(`SELECT passwordhash, university FROM users a LEFT JOIN universities b 
    ...
    bcrypt.compare(password, passwordRetrieved, function(error, res) {
      if(!res || error){
        ...
        const token = jwt.sign({username: username, uni: uni},process.env.SECRET)
...
Router.get('/authenticate', async (request, response, next) => {
  try {
    ...
    return response.status(200).send({username: decodedToken.username, uni: decodedToken.uni});
    } 
  catch(error) {
    response.status(401).send({error: error.message})
      next(error)
